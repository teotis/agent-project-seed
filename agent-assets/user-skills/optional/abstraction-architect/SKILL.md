...
description: >
  ...
  以工程证据、反例、迁移接缝和可证伪测试为基础，寻找能删除整类特殊情况的抽象机会，并产出交互式 HTML 架构报告（默认不再产出 Markdown 源报告，除非用户明确要求 agent 接力或源文件交付）。
...
核心高级动作是 **process spatialization**：当复杂度表现为时间、顺序、状态漂移、重试、审批、编排或环境依赖行为时，追问这些动态是否应该被表示为一个 canonical structural object；文档、日志、UI 状态、ledgers、prompts、reports 都只是它的一致 projections。

这个动作不是默认结论。Before searching for a canonical object，必须先通过 **no new abstraction / local deletion wins** 基线：问清楚是否只需 delete obsolete branches、merge local duplication、improve interaction copy，或保留 status quo with evidence。只有当这些低抽象候选无法解释同一 exception family、projection drift 或 workflow burden，且证据显示存在可命名 invariant 时，才进入 canonical model 或 process spatialization。

默认交付物是 **interactive HTML architecture report**，给用户审阅、比较和反馈；不默认生成 Markdown 源报告，只有用户明确要求 agent 接力、源文件交付、或环境无法产出 HTML 时才补 Markdown。本 skill 默认只做分析，不修改生产代码、测试、配置、迁移或基础设施。只有用户在审阅 transition plan 后另行明确授权，才可进入实现。
...
- **Adaptive heuristics**：process spatialization、base-change、projection、canonical object、discovery patterns 和报告形态是候选生成工具，不是固定路线。证据更支持局部删除、交互文案、边界收缩或 status quo 时，应优先保留这些低抽象答案。
...
Candidate competition 是主机制，不是附加章节。推荐任何结构前，至少比较低抽象 baseline、一个主结构候选和一个 rival explanation；证据不足时应选择 `unproven`、`defer` 或 `local deletion wins`，而不是按方法论偏好选最宏大的抽象。

## 默认执行强度与询问门槛

本 skill 同时支持用户显式调用与 agent active 自启用。默认深度由调用来源决定：

- **用户显式调用**（用户点名本 skill、提出架构审查/抽象分析诉求、或同义触发词）：默认 **Deep** 深度，跑完整结构调查、admissibility gate、candidate competition 与正式 HTML 报告。
...
|---|---|---|---|
| Low-Budget Structural Scan | Agent active 自启用默认；或用户明确要求轻量扫描；或证据不足以支持完整分析 | 简短诊断、candidate map、coverage debt 和 handoff 建议；用户要求时才升级 HTML report | No code changes |
| Full Architecture Analysis (Deep) | 用户显式调用默认 | Interactive HTML architecture report | No code changes |
| Exhaustive Architecture Analysis | 用户明确要求 "极致 / 全覆盖 / 大水漫灌" 等关键词 | Interactive HTML report + 多证据面交叉、历史/跨仓库变种、并行调查 | No code changes |
...
- **baseline before abstraction**：candidate competition 必须包含 no new abstraction / local deletion wins 基线；局部删除、合并重复、删除旧分支或文案/交互澄清能消除痛点时，优先报告这些结果，而不是升级为 canonical object。
- **model-discovered candidates**：模型临时提出的新结构候选必须进入同一 competition table，按 evidence、deleted complexity、preserved differences、counterexamples、separating probes 和 transition seam 评估；不得因为候选未在 reference 中预定义而自动排除。
...
- **最终回复**：列出报告路径和可点击 `file://` URL、说明是否仅提供预览链接或按用户要求打开 HTML、用一句话概括最高杠杆结构机会，并声明没有修改代码。
...
- HTML 交互报告包含可点击章节索引；最终回复提供报告路径和可点击 `file://` URL，主动打开只作为用户要求或明确 GUI 环境下的可选预览。
